...
create or replace package ELOG_WEBPAGE is
...
/
create or replace package body ELOG_WEBPAGE is
procedure signUp is
begin
htp.p('<html>
<head>
...
<body>
      <h1 align="center"><font size="+3" color="9900FF" face="Times New Roman,Times,serif"><b><i>ONLINE SHOPPING SYSTEM</i></b></font></h1>
      <form action="btspgsup.ELOG_USERDETAILS.signUp" method="post">
...
<body>
      ...
      <p><a href="btspgsup.ELOG_WEBPAGE.indexpage"><h3 align="center">Home Page</h3></a></p>
</body>
</html>');
end signupsuccess;
...
css;
htp.p('</head>
<body bgcolor="#00FFFF" onLoad="noBack();"
onpageshow="if (event.persisted) noBack();" onUnload="">

<center>
<span class="alert style2" ><h1 align="center"><font size="+3" color="9900FF" face="Times New Roman,Times,serif"><b><i>ONLINE SHOPPING SYSTEM</i></b></font></h1>
</span>
<form action="btspgsup.ELOG_USERDETAILS.VerifyUser" method="post">
<center><br><br><br><br><br><br>
<center><font size="5" color="9900FF" face="Times New Roman,Times,serif"><i><b>Sign In</b></i></center>
...
<input  type = submit name= click value = login class=buttonm>
   ...
   <a href="btspgsup.ELOG_WEBPAGE.signUp"><center><i><font size="5" color="9900FF" face="Times New Roman,Times,serif"><b>SignUp</b></font></i></center></a>
</form>
</center>
</body>
</html>');
...
.buttonm {
  BORDER-RIGHT: #eeeeee 1px solid; BORDER-TOP: #eeeeee 1px solid; FONT-WEIGHT: bold; FONT-SIZE: 8pt; BORDER-LEFT: #7a7a7a 1px solid; CURSOR: hand; COLOR: #DEE7CA; BORDER-BOTTOM: #7a7a7a 1px solid; FONT-FAMILY: Verdana, Arial, Helvetica, sans-serif; BACKGROUND-COLOR: #191970
}
.alert {
  FONT-WEIGHT: bold; FONT-SIZE: 20pt; COLOR:#8A2BE2; FONT-FAMILY:Monotype Corsiva; TEXT-DECORATION: none
}
.star {
...
DIV.tab {
  BORDER-RIGHT: #000000 1px solid; PADDING-RIGHT: 5px; BORDER-TOP: #ffffff 1px solid; PADDING-LEFT: 5px; PADDING-BOTTOM: 5px; BORDER-LEFT: #ffffff 1px solid; WIDTH: 100%; PADDING-TOP: 8px; BORDER-BOTTOM: #000000 1px solid; HEIGHT: 100%; BACKGROUND-COLOR: #191970
}
TABLE.tabInset {
...
.on {
  FONT-WEIGHT: bold; FONT-SIZE: 8pt; PADDING-TOP: 2px; TOP: 1px; HEIGHT: 20px; BACKGROUND-COLOR: #191970
}
.off {
...
.table_bluecell {background-color:#D7FDFF;  font-family:Verdana, Arial, Helvetica, sans-serif; color:#003000; font-size:11px; font-weight:normal;  border-right:1px solid #407B61; border-bottom:0px solid #777766;}
...
<body bgcolor="#00FFFF" onload="focus()">

<center>  ');
htp.p('<font size="6" face="serif" color="9900FF"> SQL Manipulation - Usage of Bind variables');
htp.p('<form action="btspgsup.ELOG_USERDETAILS.VerifyUser" method="post" name =form1>
...
begin
  ELOG_USERDETAILS.g_orderlist :=ELOG_USERDETAILS.orderlist();
  ELOG_USERDETAILS.g_cfplist:= ELOG_USERDETAILS.cfplist();
  ELOG_USERDETAILS.gv_order:=ELOG_USERDETAILS.orderlist();
end initialisevar;*/
...
<body onLoad="noBack();"
onpageshow="if (event.persisted) noBack();" onUnload="" bgcolor="#00FFFF">
<p align="right">
<a href="btspgsup.ELOG_WEBPAGE.indexpage"><input type="button" class="btn" value="Logout" ></a></p></font>
<p align="center">
<span class="alert style2">Welcome Manager</span><font color="#00FFFF">
');
--ELOG_WEBPAGE.searchorder;
htp.p('<table width="737" border="0"  align="center">
 <form action="btspgsup.ELOG_WEBPAGE.insertporductpage" method="post" name="form1">
    ...
    <form action="btspgsup.ELOG_WEBPAGE.searchorder" method="post" name="form2">
        <tr class="table_bluecell">
         <td height="32" align="left" valign="middle" colspan="5"><p>
...
htp.p('<font  class="alert style2">Order List</font>
...
<table width="738" height="52" border="0"  align="center">
  ...
  for i in 1 .. ELOG_USERDETAILS.G_ORDERLIST.count loop
  if ELOG_USERDETAILS.g_orderlist.exists(i) then
    htp.p('<tr class="table_bluecell">
    <td>'||ELOG_USERDETAILS.g_orderlist(i).order_id||'</td>
    <td>'||ELOG_USERDETAILS.g_orderlist(i).CUSTOMER_ID||'</td>
    <td>'||ELOG_USERDETAILS.g_orderlist(i).PRODUCT ||'</td>
    <td>'||ELOG_USERDETAILS.g_orderlist(i).PROD_TYPE||'</td>
    <td>'||ELOG_USERDETAILS.g_orderlist(i).PNAME ||'</td>
    <td>'||ELOG_USERDETAILS.g_orderlist(i).QUNATITY||'</td>
    <td>'||ELOG_USERDETAILS.g_orderlist(i).DESTINATION||'</td>
    <td>'||ELOG_USERDETAILS.g_orderlist(i).STATUS ||'</td>
  </tr> ');
  end if;
  end loop;
htp.p('
...
<table width="738" height="52" border="0"  align="center">
  ...
  ELOG_USERDETAILS.getCfpDetails;
  for i in 1 .. ELOG_USERDETAILS.g_cfplist.count loop
  if ELOG_USERDETAILS.g_cfplist.exists(i) then
    htp.p('<tr class="table_bluecell">
    <td>'||ELOG_USERDETAILS.g_cfplist(i).CFP_ID||'</td>
    <td>'||ELOG_USERDETAILS.g_cfplist(i).ORDER_ID    ||'</td>
    <td>'||ELOG_USERDETAILS.g_cfplist(i).SUPPLIER_ID  ||'</td>
    <td>'||ELOG_USERDETAILS.g_cfplist(i).PRODUCT     ||'</td>
    <td>'||ELOG_USERDETAILS.g_cfplist(i).QUANTITY     ||'</td>
    <td>'||ELOG_USERDETAILS.g_cfplist(i).STATUS      ||'</td>
  </tr> ');
  end if;
  end loop;
htp.p('
...
<style type="text/css">
<!--
.style2 {font-size: 15pt FONT-WEIGHT: bold;}
-->
</style>
<SCRIPT type="text/javascript">
window.history.forward();
...
<body onLoad="noBack();"
onpageshow="if (event.persisted) noBack();" onUnload=""  bgcolor="#00FFFF">');
htp.p('<center><font size="6" face="serif" color="9900FF"> Code Injection - Usage of DBMS_ASSERT package</center>');
htp.p('
<p align="right">
<a href="btspgsup.ELOG_WEBPAGE.indexpage"><input type="button" class="btn" value="Logout" ></a></p>
<center><span class="alert style2">Welcome Customer</span></center>
  <table width="737" border="0" align="center">
      <form action="btspgsup.ELOG_USERDETAILS.PlaceOrder" method="post" name="form1">
    <tr>
    ...
    <tr class="table_bluecell">
      ...
      <form method="post" action="btspgsup.ELOG_WEBPAGE.LoginPage" name="form2">
      </td></form>
 </tr>
  </table>
...
<table width="738" height="52" border="0" align="center">
  ...
  ELOG_USERDETAILS.getOrderDetails(customer_name);
  for i in 1 .. ELOG_USERDETAILS.G_ORDERLIST.count loop
  if ELOG_USERDETAILS.g_orderlist.exists(i) then
    htp.p('<tr class="table_bluecell">
    <td>'||ELOG_USERDETAILS.g_orderlist(i).order_id||'</td>
...
onpageshow="if (event.persisted) noBack();" onUnload="" bgcolor="#00FFFF">');
ELOG_USERDETAILS.getCfpDetails;
if (ELOG_USERDETAILS.g_cfplist.count>0) then
htp.p('<p align="right">
<a href="btspgsup.ELOG_WEBPAGE.indexpage"><input type="button" class="btn" value="Logout" ></a></p>
<p class="alert style1" align="left">Welcome Supplier</p><p align="center" class="alert style1">Call For Proposals</p>
<table width="738" height="52" border="0" align="center">
  <tr class="buttonm">
    <td width="83"><div align="center">Cfp Id </div></td>
    <td width="84"><div align="center">Order ID</div></td>
...
/*  <%  for( int i=0;i< al.size();i++)
  ...
  <tr class="table_bluecell">
    ...
    <td><% if((CB.getStatus()).equals("waiting")) { %><a href="ELOG_USERDETAILS.AcceptOrder?index=<%=CB.getOrder_id()%>"><%=CB.getStatus()%></a><% } else { %><%=CB.getStatus()%><% } %></td>
  </tr>
<%}
}
%>*/
  ...
  if ELOG_USERDETAILS.g_cfplist.exists(i) then
    ...
    <td>'||ELOG_USERDETAILS.g_cfplist(i).QUANTITY     ||'</td>
    <td>');
    if ELOG_USERDETAILS.g_cfplist(i).STATUS = 'waiting'  then
   htp.p('<a href="btspgsup.ELOG_USERDETAILS.AcceptOrder?inde='||ELOG_USERDETAILS.g_cfplist(i).ORDER_ID ||'">'
   ||ELOG_USERDETAILS.g_cfplist(i).STATUS
    ||'</a>');
    else

         htp.p(ELOG_USERDETAILS.g_cfplist(i).STATUS||'</td>
  </tr> ');
end if;
end if;
end loop;
...
IF ELOG_USERDETAILS.gv_order.EXISTS(1) THEN
htp.p('</table>
 <p class="alert style1" align="center">Order Details </p>
 <form name="form1" method="post" action="btspgsup.ELOG_USERDETAILS.AcceptOrder">
 <input name="index1" type="hidden" value="'||ELOG_USERDETAILS.gv_order(1).order_id||'">
 <table width="718" border="0" align="center">
   <tr>
     <td width="207" class="buttonm">Order ID </td>
     <td width="495" class="table_bluecell" >'||ELOG_USERDETAILS.gv_order(1).order_id||'</td>
   </tr>
   <tr>
     <td class="buttonm">Customer ID</td>
     <td class="table_bluecell">'||ELOG_USERDETAILS.gv_order(1).CUSTOMER_id||'</td>
   </tr>
   <tr>
     <td class="buttonm">Product</td>
     <td class="table_bluecell">'||ELOG_USERDETAILS.gv_order(1).PRODUCT||'</td>
   </tr>
   <tr>
     <td class="buttonm">Type</td>
     <td class="table_bluecell">'||ELOG_USERDETAILS.gv_order(1).PROD_TYPE||'</td>
   </tr>
   <tr>
     <td class="buttonm">Product Name</td>
     <td class="table_bluecell">'||ELOG_USERDETAILS.gv_order(1).PNAME||'</td>
   </tr>
   <tr>
     <td class="buttonm">Quantity</td>
     <td class="table_bluecell">'||ELOG_USERDETAILS.gv_order(1).QUNATITY||'</td>
   </tr>
   <tr>
     <td class="buttonm">Destination</td>
     <td class="table_bluecell">'||ELOG_USERDETAILS.gv_order(1).DESTINATION||'</td>
   </tr>
   <tr>
...
<style type="text/css">
<!--
.style1 {font-size: 20pt;FONT-WEIGHT:bold; }
-->
</style>
<SCRIPT type="text/javascript">
window.history.forward();
...
onpageshow="if (event.persisted) noBack();" onUnload="" bgcolor="#00FFFF">');
  ELOG_USERDETAILS.getOrderDetails;
  if ELOG_USERDETAILS.G_ORDERLIST.count !=0 then
  htp.p('<p align="right"><a href="btspgsup.ELOG_WEBPAGE.indexpage"><input type="button" class="btn" value="Logout" ></a></p>
<p class="alert style1" align="center">Order Details </p>
<table width="738" height="52" border="0" align="center">
<table width="738" height="52" border="0" align="center">>
  <tr class="buttonm">
  ...
  if ELOG_USERDETAILS.g_orderlist.exists(i) then
    ...
    <td>'||ELOG_USERDETAILS.g_orderlist(i).PNAME ||'</td>
    <td>'||ELOG_USERDETAILS.g_orderlist(i).QUNATITY||'</td>
    <td>'||ELOG_USERDETAILS.g_orderlist(i).DESTINATION||'</td><td>');
      if ELOG_USERDETAILS.g_orderlist(i).STATUS = 'Accept'  then
   htp.p('<a href="btspgsup.ELOG_USERDETAILS.MoveOrder?inde='||ELOG_USERDETAILS.g_orderlist(i).ORDER_ID ||'">'
   ||ELOG_USERDETAILS.g_orderlist(i).STATUS
    ||'</a></td>');
    else

         htp.p(ELOG_USERDETAILS.g_orderlist(i).STATUS||'</td>
  </tr> ');
end if;
  end if;
  end loop;
...
IF ELOG_USERDETAILS.gv_order.EXISTS(1) THEN
 htp.p('
 <form name="form1" method="post" action="btspgsup.ELOG_USERDETAILS.MoveOrder">
 <input name="index1" type="hidden" value="'||ELOG_USERDETAILS.gv_order(1).order_id||'">
...
heading1 varchar2(300) :=null;
begin

htp.p('<html>
<head>');
htp.p('<title>E - Logistic For Supply Chain Management</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1"> ');
css;
htp.p('
<SCRIPT language="JavaScript" type="text/javascript">');
...
htp.p('function changeHeading()
  {document.write(asdfg)

  alert(formObject.search_field.options[indexList].value)
var formObject = document.form1
var indexList = formObject.search_field.selectedIndex
if((formObject.search_field.options[indexList].value)==("ORDER_ID"))
{ alert(hi)');
--heading1 := 'Function Call Injection - usage of authid Current_user';
htp.p('}');
htp.p('else
{ alert(hiii)');
--heading1 := 'Access Secured Information - usage of DBMS_DDL package';
htp.p('}
...
<body bgcolor="#00FFFF">');
htp.p('<center><font size="6" class="alert style2">'|| heading1||'</center></br>');
htp.p('
<span class="alert style2">Welcome Manager</span>

  <table width="737" border="0">
      <form action="btspgsup.ELOG_USERDETAILS.SearchOrder" method="post" name="form1">
    <tr>
    ...
    <tr class="table_bluecell">
      <td height="32" align="left" valign="middle"><p>
        <select name="search_field" class="textfield"  onChange="changeHeading()">');
          htp.p('  <option>Select Search Feild</option>
      ...
      <td>Enter Value</td>
      <td><input name="fieldvalue" type="text" class="textfield" size="15" id ="fieldvalue"></td>
      ...
      <form method="post" action="btspgsup.ELOG_WEBPAGE.manager" name="form2">
...
<table width="738" height="52" border="0">
 ...
 --ELOG_USERDETAILS.getOrderDetails;
  ...
  htp.p('<font size="6" class="alert style2">You have no orders</font>');
  end if;
  htp.p('</body></html>');
end searchorder;
...
<body bgcolor="#00FFFF">
 ');
--htp.p('<center><font size="6" face="serif" color="GOLD"> Code Injection </center></br>');
htp.p('<table width="737" border="0" align="center">
 <form action="btspgsup.ELOG_USERDETAILS.insertproduct" method="post" name="form1">
        <tr>
